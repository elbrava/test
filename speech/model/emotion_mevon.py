...
def predict(file, classes, model):
    ...
    X, sample_rate = librosa.load(os.path.join(f'{folder}{"/"}{subdir}{"/"}', file), res_type='kaiser_fast',
    ...
    for subdir in os.listdir(folder):
        ...
        with open('SER_' + subdir + '.csv', 'w') as csvFile:
            ...
            for i in range(len(filenames)):
                ...
                print("filename:", filenames[i], ",Predicted Emotion := Person1:", predictions[i][0], ",Person2:",
                      predictions[i][1])
